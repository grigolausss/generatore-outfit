...
class OutfitGeneratorApp:
    ...
    def create_widgets(self):
        ...
        # Gestione dello scorrimento con il touchpad
        def on_mouse_wheel(event):
            delta = 0
            if event.delta:
                delta = -event.delta // 120  # Normalizza per macOS
            elif event.num == 4:
                delta = -1
            elif event.num == 5:
                delta = 1
        ...
        folders_frame = tk.LabelFrame(scrollable_frame, text="📂 Impostazioni Cartelle", font=("Helvetica", 14, "bold"), bg="#ffffff", fg="#333333", padx=10, pady=10)
        folders_frame.pack(fill="x", padx=20, pady=10)

        for idx, item in enumerate(self.accessory_types):
            folder_inner_frame = tk.Frame(folders_frame, bg="#ffffff")
        ...
        tk.Label(accessory_count_frame, text="🔢 Numero accessori (0 a max):", width=25, anchor="w", bg="#ffffff").pack(side="left")
        self.accessory_slider = tk.Scale(accessory_count_frame, from_=0, to=len([cat for cat in self.accessory_types if cat not in ["maglie", "pantaloni", "sfondi"]]), orient="horizontal", length=300)
    ...
    def random_accessory_count(self):
        ...
        self.accessory_slider.set(safe_randint(0, max_accessories))
    ...
    def load_images_from_folder(self, folder_path):
        ...
        for file in os.listdir(folder_path):
            if file.lower().endswith(valid_extensions):
                ...
                images.append((full_path, os.path.splitext(file)[0]))  # Salva il percorso e il nome base
        return images
    ...
    def is_position_safe(self, x, y, width, height, img_width, img_height):
        # Esclude 75 pixel dall'alto, 125 pixel dal basso, e 150 pixel dalla destra
        safe_width = width - 150  # Esclude la barra destra di TikTok (~150px)
        safe_height = height - 75 - 125  # Esclude 75px sopra e 125px sotto
        safe_left = 0
        safe_top = 75
    ...
    def generate_outfits(self):
        try:
            ...
            max_accessories = self.accessory_slider.get()
            ...
            for i in range(quantity):
                ...
                # Scegli uno sfondo: 60% tinta unita, 40% immagine
                ...
                shirt_max_width = int((width - 150) * 0.5 * size_main_factor)  # Considera la safe width
                shirt_max_height = int((height - 200) * 0.35 * size_main_factor)  # Considera la safe height (75 + 125)
                ...
                accessory_max_width = int((width - 150) * 0.25 * size_accessory_factor)
                accessory_max_height = int((height - 200) * 0.25 * size_accessory_factor)

                # Carica e ridimensiona maglia e pantaloni con nomi
                ...
                # Posizionamento casuale con offset per maglie e pantaloni
                max_offset_x = int((width - 150) * 0.2)  # Spostamento massimo del 20% della safe width
                offset_x = safe_randint(-max_offset_x, max_offset_x)
                shirt_x = ((width - 150) - shirt_img.width) // 2 + offset_x + 0  # Inizia dalla safe zone
                shirt_y = 75 + int((height - 200 - shirt_img.height) * 0.25)  # Inizia dopo 75px dall'alto
                shirt_x = max(0, min(width - 150 - shirt_img.width, shirt_x))  # Limita ai bordi sicuri
                ...
                if self.show_names.get():
                    ...
                    if text_x + text_width > width - 150:  # Evita margine destro
                        text_x = width - 150 - text_width
                    if text_y + font_size > height - 125:  # Evita margine inferiore
                ...
                pants_x = ((width - 150) - pants_img.width) // 2 + offset_x
                pants_y = shirt_y + shirt_img.height + spacing
                pants_x = max(0, min(width - 150 - pants_img.width, pants_x))  # Limita ai bordi sicuri
                if pants_y + pants_img.height > height - 125:  # Evita margine inferiore
                ...
                # Determina il lato opposto per gli accessori
                side_preference = "right" if offset_x < 0 else "left"
                accessory_positions = []

                for _ in range(min(4, len(selected_accessories))):
                    if side_preference == "right":
                        x_base = int((width - 150) * 0.6)
                        y_base = 75 + int((height - 200) * 0.3) + safe_randint(-100, 100)
                    else:
                        x_base = int((width - 150) * 0.2)
                        y_base = 75 + int((height - 200) * 0.3) + safe_randint(-100, 100)
                    accessory_positions.append((x_base, y_base))

                for idx, (accessory, pos) in enumerate(zip(selected_accessories, accessory_positions[:len(selected_accessories)])):
                    item_data = random.choice(category_images[accessory])
                    item_img = fix_image_orientation(Image.open(item_data[0]).convert("RGBA"))
                    item_img = self.resize_image(item_img, accessory_max_width, accessory_max_height)
                    ...
                    while attempts < max_attempts:
                        x_var = safe_randint(-50, 50)
                        y_var = safe_randint(-50, 50)
                        x = max(0, min(width - 150 - item_img.width, x + x_var))
                        y = max(75, min(height - 125 - item_img.height, y + y_var))
                        ...
                        if not overlaps and self.is_position_safe(x, y, width, height, item_img.width, item_img.height):
                            placed = True
                            break
                        attempts += 1
                    ...
                    if self.show_names.get():
                        ...
                        if text_y + font_size > height - 125:
                            text_y = height - 125 - font_size
                        font_color = self.get_font_color()
                        draw.text((text_x, text_y), item_name, font=font, fill=font_color)
                        occupied_areas.append((text_x, text_y, text_width, font_size))
